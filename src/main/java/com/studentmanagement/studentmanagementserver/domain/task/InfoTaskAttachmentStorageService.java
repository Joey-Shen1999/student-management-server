...
@Service
public class InfoTaskAttachmentStorageService {

    private static final String NAMESPACE = "info-task-attachments";

    private final Path rootDirectory;
    private final com.studentmanagement.studentmanagementserver.domain.student.StudentFileStorageBackend storageBackend;

    public InfoTaskAttachmentStorageService(
            @Value("${app.info-task.attachments.file-dir:uploads/info-task-attachments}") String rootDir,
            com.studentmanagement.studentmanagementserver.domain.student.StudentFileStorageBackend storageBackend
    ) {
        this.rootDirectory = Paths.get(rootDir).toAbsolutePath().normalize();
        this.storageBackend = storageBackend;
    }

    public StoredAttachment store(Long infoTaskId, MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new IllegalArgumentException("attachment file is required");
        }
        if (infoTaskId == null || infoTaskId.longValue() <= 0L) {
            throw new IllegalArgumentException("infoTaskId must be positive");
        }

        String originalFilename = sanitizeOriginalFilename(file.getOriginalFilename());
        String extension = extractExtension(originalFilename);
        String storageKey = "info-" + infoTaskId
                + "_attachment-"
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            throw new IllegalStateException("Failed to store info attachment file");
        }
    ...
    public byte[] readAllBytes(String storageKey) {
        return storageBackend.readAllBytes(NAMESPACE, rootDirectory, storageKey, "Info attachment file not found.");
    }
    ...
    public static class StoredAttachment {
        ...
        private StoredAttachment(String storageKey,
