-- DUOLINGO language score type support and historical data backfill

-- Backfill by score range: any record above IELTS max implies DUOLINGO.
...
-- Backfill legacy sources if old test type columns still exist in some environments.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
WHERE language_score_type IS NULL
   OR language_score_type NOT IN ('IELTS', 'TOEFL', 'DUOLINGO');
...
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE student_ielts_module
            DROP CONSTRAINT ck_student_ielts_module_language_score_type;
    END IF;
END $$;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE student_ielts_module
            ADD CONSTRAINT ck_student_ielts_module_language_score_type
                CHECK (language_score_type IN ('IELTS', 'TOEFL', 'DUOLINGO'));
    END IF;
END $$;
